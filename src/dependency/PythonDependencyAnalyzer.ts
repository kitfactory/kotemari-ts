...
export class PythonDependencyAnalyzer implements IDependencyAnalyzer {
  analyze(projectRoot: string, exclude: string[] = []) {
    let files = fs.readdirSync(projectRoot).filter(f => f.endsWith('.py'));
    if (exclude.length > 0) {
    ...
    for (const file of files) {
      ...
      // Python: import xxx or from xxx import yyy
      const deps = Array.from(content.matchAll(/(?:import|from)\s+([\w_\.]+)/g)).map(m => m[1] + '.py');
...
export const pythonDependencyAnalyzer = new PythonDependencyAnalyzer();

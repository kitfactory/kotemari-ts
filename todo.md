...
- [x] 最大コンテキスト長のパラメータ設計
    - [x] Kotemariクラスの初期化オプションにmaxContextLength（最大トークン数）を追加
        - [x] テスト実装
        - [x] 実装
        - [x] テストがパス
    - [x] 設定ファイル（例: .kotemari.yml）からも最大長を指定可能にする
        - [x] テスト実装
        - [x] 実装
        - [x] テストがパス
    - [ ] APIドキュメント・READMEへの反映
        ...
        - [ ] レビュー
- [x] コンテキスト長制御ロジックの実装
    - [x] 依存ファイルの連結時に最大長を超えないようトリミング・優先順位制御
        - [x] テスト実装
        - [x] 実装
        - [x] テストがパス
    - [x] トークン数カウントのためのユーティリティ実装（単純文字数/サブワード/実トークン数など選択肢検討）
        - [x] テスト実装
        - [x] 実装
        - [x] テストがパス
    - [x] テストケース作成（長い依存を持つ場合の動作検証）
        - [x] テスト実装
        - [x] 実装
        - [x] テストがパス
- [x] 利用モードごとのポリシー切り替え
    - [x] ローカルLLM/クラウドLLM/API課金有無ごとに推奨コンテキスト長をプリセット
        - [x] テスト実装
        - [x] 実装
        - [x] テストがパス
    - [x] 利用モード選択用の設定・オプション追加
        - [x] テスト実装
        - [x] 実装
        - [x] テストがパス
    - [x] モードごとに最大長・優先順位制御が切り替わることのテスト
        - [x] テスト実装
        - [x] 実装
        - [x] テストがパス
- [ ] コンテキストキャッシュの一貫性担保
    - [x] 同一入力・依存関係で常に前方から同一のコンテキストが生成されるアルゴリズム設計
        - [設計方針]
            - 依存ファイルをたどる際、重複を排除し（Set化）、さらにファイル名でソートして順序を安定化させる。
            - これにより、依存解決経路やimport文の記述順にかかわらず、同じ依存集合なら常に同じファイルリストとなる。
            - 依存ファイルリストの共通プレフィックス部分（例：A,B）は、複数ターゲット間で文脈断片やキャッシュとして共有できる。
            - 依存ファイルや内容が変わらない限り、キャッシュキーや文脈も一貫性が保たれる。
            - 依存ファイルの内容が異なる場合（import文の重複や順序違い含む）は、キャッシュキー・文脈も異なる（仕様通り）。
        - [ ] テスト実装
        - [ ] 実装
        - [ ] テストがパス
    - [x] キャッシュキー生成・管理ロジックの実装
        - [x] テスト実装
        - [x] 実装
        - [x] テストがパス
        - [x] テストがパス
